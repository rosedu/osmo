...
gboolean repeat, prev_state, next_state;
    ...
    if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
		...
		if (repeat == TRUE) {
			prev_state = FALSE;
			next_state = is_date_in_the_past (due_date_julian, due_time, 
											  utl_get_current_julian (), get_seconds_for_today ());
		} else {
...
gint time;
    ...
    while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(appGUI->tsk->tasks_list_store), &iter, NULL, i++)) {
        ...
        if (get_priority_index (priority) == HIGH_PRIORITY && config.tasks_high_in_bold == TRUE) {    /* high priority ? */
            gtk_list_store_set(appGUI->tsk->tasks_list_store, &iter, 
...
store_task_columns_info (GUI *appGUI) {
	...
	config.tasks_column_idx_0_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 0));
	config.tasks_column_idx_1_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 1));
	config.tasks_column_idx_2_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 2));
	config.tasks_column_idx_3_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 3));
	config.tasks_column_idx_4_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 4));
	config.tasks_column_idx_5_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 5));
}
...
guint n_entries = G_N_ELEMENTS (entries);
	...
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 0),
										  config.tasks_column_idx_0_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 1),
										  config.tasks_column_idx_1_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 2),
										  config.tasks_column_idx_2_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 3),
										  config.tasks_column_idx_3_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 4),
										  config.tasks_column_idx_4_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->tsk->tasks_list), 5),
										  config.tasks_column_idx_5_width - 2 * utl_gui_get_sw_vscrollbar_width (top_scrolledwindow));
