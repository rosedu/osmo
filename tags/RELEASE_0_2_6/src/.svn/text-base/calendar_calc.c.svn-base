...
{
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (year), utl_get_current_year ());
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (month), utl_get_current_month ());
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (day), utl_get_current_day ());
}
...
set_result2_cb (GtkWidget *widget, gpointer user_data)
{
    ...
    /* add */
        ...
        julian = utl_dmy_to_julian (day, month + 1, year);
	...
	if (year < 1) {
		...
		if (hour == 0 && minute == 0 && second == 0) {
			...
			g_snprintf (timebuf, BUFFER_SIZE, "%02d:%02d.%02d", (gint)hour, (gint)minute, (gint)second);
...
char      tmpbuf[BUFFER_SIZE];
    ...
    spinbutton_adjustment = gtk_adjustment_new (0, 0, 999999, 1, 10, 0);
