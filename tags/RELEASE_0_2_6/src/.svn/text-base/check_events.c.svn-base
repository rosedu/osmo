			...
			if (iter != NULL) {
				...
				if (a->item->repeat == TRUE) {
					...
					gtk_list_store_set (appGUI->tsk->tasks_list_store, iter,
					                    TA_COLUMN_COLOR, get_date_color (a->item->due_date_julian, a->item->due_time, TRUE, appGUI),
...
guint i;
	...
	for (i = 0, tnode = appGUI->tsk->notifications; tnode != NULL; tnode = tnode->next, i++) {
		...
		if (a->notify == n) {
			...
			a->time = get_seconds_for_today () + a->item->postpone_time * 60;
...
guint i, id;
	...
	/* FIXME: alarm warning bug */
	notify_notification_clear_actions (a->notify);
	if (a->item->postpone_time > 0) {
...
guint32             i, j, start_date, current_date, julian_day;
gboolean            day_notes_flag, tasks_flag;
GSList              *lnode;
