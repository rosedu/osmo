...
gchar tmpbuf[BUFFER_SIZE];
    ...
    if (mode == SELECTOR) { 
        ...
        gtk_tree_model_get (GTK_TREE_MODEL(appGUI->nte->notes_list_store), &n_iter, 
							...
							N_COLUMN_REMEMBER_EDITOR_LINE, &remember_cursor, -1);
...
store_note_columns_info (GUI *appGUI) {
	...
	config.notes_column_idx_0_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 0));
	config.notes_column_idx_1_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 1));
	config.notes_column_idx_2_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 2));
	config.notes_column_idx_3_width = gtk_tree_view_column_get_width(gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 3));
}
...
gint nt_columns[MAX_VISIBLE_NOTE_COLUMNS] = { 
		N_COLUMN_TYPE, N_COLUMN_NAME, N_COLUMN_CATEGORY, N_COLUMN_LAST_CHANGES_DATE
};
...
guint n_t_entries_editor = G_N_ELEMENTS (t_entries_editor);
...
#ifdef HAVE_GTKSPELL
    gtk_action_group_add_toggle_actions (action_group_editor, t_entries_editor, n_t_entries_editor, NULL);
#endif /* HAVE_GTKSPELL */
    ...
    appGUI->nte->notes_list_store = gtk_list_store_new(NOTES_NUM_COLUMNS, 
                                                       ...
                                                       G_TYPE_STRING, G_TYPE_UINT, G_TYPE_UINT,
                                                       G_TYPE_BOOLEAN, G_TYPE_UINT, G_TYPE_STRING);
    ...
    renderer = gtk_cell_renderer_text_new();
    appGUI->nte->notes_columns[N_COLUMN_CREATE_DATE] = gtk_tree_view_column_new_with_attributes(NULL,
																								renderer,
																								"text", N_COLUMN_CREATE_DATE,
																								NULL);
    gtk_tree_view_column_set_visible (appGUI->nte->notes_columns[N_COLUMN_CREATE_DATE], FALSE);
	...
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 0),
										  config.notes_column_idx_0_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 1),
										  config.notes_column_idx_1_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 2),
										  config.notes_column_idx_2_width);
	gtk_tree_view_column_set_fixed_width (gtk_tree_view_get_column (GTK_TREE_VIEW(appGUI->nte->notes_list), 3),
										  config.notes_column_idx_3_width);
