...
extern guint32      tasks_due_julian_day;
...
GtkListStore        *tasks_list_store = NULL;
gboolean            tasks_panel_status = FALSE;
...
gint                filter_index;
gboolean            tasks_filter_disabled = FALSE;
GtkUIManager        *tasks_uim_widget = NULL;
...
gint
priority_combo_box_get_text_index (gchar *text) {
...
void
show_tasks_desc_panel (gboolean enable) {
...
GtkTreeModel *model;
    ...
    if(enable == TRUE) {
        ...
        if (gtk_tree_selection_get_selected (tasks_list_selection, &model, &iter)) {
 
            gtk_paned_set_position(GTK_PANED(tasks_paned), config.tasks_pane_pos);
            ...
            if (visible_path) {
                ...
                gtk_tree_view_get_cell_area (GTK_TREE_VIEW (tasks_list), visible_path, NULL, &rect);
                gtk_tree_view_get_visible_rect (GTK_TREE_VIEW (tasks_list), &visible_rect);

                if (rect.y < visible_rect.y || rect.y > visible_rect.y + visible_rect.height) {
                        gtk_tree_view_scroll_to_cell (GTK_TREE_VIEW (tasks_list), visible_path, NULL, TRUE, 0.5, 0.0);
                }
        ...
        config.tasks_pane_pos = gtk_paned_get_position(GTK_PANED(tasks_paned));
        gtk_paned_set_position(GTK_PANED(tasks_paned), 99999);

    }

    tasks_panel_status = enable;
}

/*------------------------------------------------------------------------------*/

void 
fill_fields (GtkWidget *due_date_entry, GtkWidget *summary_entry, GtkWidget *desc_entry) {
        ...
        if (filter_path != NULL) {
            ...
            if (path != NULL) {
                ...
                gtk_tree_model_get (GTK_TREE_MODEL(tasks_list_store), &iter, 
                           ...
                           COLUMN_DUE_DATE_JULIAN, &tasks_due_julian_day, 
                ...
                gtk_combo_box_set_active(GTK_COMBO_BOX(category_combobox), 
                                         gui_list_store_get_text_index (tasks_category_store, fcategory));
                g_free(fcategory);
                gtk_combo_box_set_active(GTK_COMBO_BOX(priority_combobox), 
                                         priority_combo_box_get_text_index (fpriority));
                g_free(fpriority);
...
guint32 start_date_julian;
    ...
    text_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(tasks_desc_textview));
    gui_clear_text_buffer (text_buffer, &titer);

    if (gtk_tree_selection_get_selected (selection, &model, &iter)) {

        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (tasks_uim_widget, "/toolbar/edit"), TRUE);
        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (tasks_uim_widget, "/toolbar/delete"), TRUE);
        ...
        if (start_date_julian) {
            ...
            gtk_text_buffer_insert_with_tags_by_name (text_buffer, &titer, tmpbuf, -1, 
                                                      "italic", NULL);
            gtk_text_view_set_buffer(GTK_TEXT_VIEW(tasks_desc_textview), text_buffer);

            hseparator = gtk_hseparator_new ();
            gtk_widget_show (hseparator);
            gtk_widget_set_size_request (hseparator, 240, -1);
            gtk_text_view_add_child_at_anchor (GTK_TEXT_VIEW(tasks_desc_textview), hseparator, anchor);
        }

        g_free(text);
    } else {
        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (tasks_uim_widget, "/toolbar/edit"), FALSE);
        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (tasks_uim_widget, "/toolbar/delete"), FALSE);
    }
}
...
guint32 julian_day;
	...
	while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(tasks_list_store), &iter, NULL, i++)) {
        ...
        gtk_tree_model_get(GTK_TREE_MODEL(tasks_list_store), &iter, COLUMN_DONE, &done, COLUMN_DUE_DATE_JULIAN, &julian_day, -1);
        gtk_list_store_set(tasks_list_store, &iter, COLUMN_COLOR, get_date_color(julian_day, done), -1);
    }
}
...
guint32 julian_day;
	...
	while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(tasks_list_store), &iter, NULL, i++)) {
        ...
        gtk_tree_model_get(GTK_TREE_MODEL(tasks_list_store), &iter, COLUMN_DUE_DATE_JULIAN, &julian_day, -1);
        gtk_list_store_set(tasks_list_store, &iter, COLUMN_DUE_DATE, julian_to_str(julian_day, config.date_format), -1);
    }
...
gint i;
	...
	while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(tasks_filter), &iter, NULL, i++));
    ...
    if (!i) {
        ...
        sprintf(tmpbuf, "<i>%4d %s</i>", 1, _("entry"));
    } else {
        sprintf(tmpbuf, "<i>%4d %s</i>", i, _("entries"));
    }

    gtk_label_set_markup (GTK_LABEL (n_items_label), tmpbuf);
...
void
add_item_to_list(gboolean done, guint32 due_date_julian, guint32 start_date_julian, gchar *priority, gchar *category, gchar *summary, gchar *desc) {

GtkTreeIter iter;

    gtk_list_store_append(tasks_list_store, &iter);

    if (due_date_julian == 0) {   /* use 'No date' text when due date is 0 */
        gtk_list_store_set(tasks_list_store, &iter, COLUMN_DONE, done, COLUMN_DUE_DATE, _("No date"), 
        ...
        gtk_list_store_set(tasks_list_store, &iter, COLUMN_DONE, done, COLUMN_DUE_DATE, julian_to_str(due_date_julian, config.date_format),
...
gchar *category;
    ...
    if(filter_index) {
        ...
        if (gui_list_store_get_text_index (tasks_category_store, category) == filter_index) {
...
category_filter_cb (GtkComboBox *widget, gpointer user_data) {
    ...
    filter_index = gtk_combo_box_get_active (GTK_COMBO_BOX (widget));
...
guint32 due_date_s, due_date_si;
    ...
    priority_s = priority_combo_box_get_text_index (priority_a) - priority_combo_box_get_text_index (priority_b);
    priority_si = priority_combo_box_get_text_index (priority_b) - priority_combo_box_get_text_index (priority_a);
    g_free(priority_a);
    g_free(priority_b);
...
void
tasks_select_first_position_in_list(void) {

GtkTreeIter     iter;
GtkTreePath     *path;

    /* set cursor at first position */
    if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(tasks_list_store), &iter) == TRUE) {
        path = gtk_tree_model_get_path (GTK_TREE_MODEL(tasks_list_store), &iter);
        if (path != NULL) {
            gtk_tree_view_set_cursor (GTK_TREE_VIEW (tasks_list), path, NULL, TRUE);
            gtk_tree_path_free(path);
        }
    }   
}
...
void
add_tasks_toolbar_widget (GtkUIManager *tasks_uim_widget, GtkWidget *widget, GtkBox *box) {

GtkWidget *handle_box;

    if (GTK_IS_TOOLBAR (widget)) {

        tasks_toolbar = GTK_TOOLBAR (widget);
...
void 
gui_create_tasks(GtkWidget *notebook) {
...
guint n_entries = G_N_ELEMENTS (entries);
    ...
    tasks_uim_widget = gtk_ui_manager_new ();

    gtk_ui_manager_insert_action_group (tasks_uim_widget, action_group, 0);
    g_signal_connect (tasks_uim_widget, "add_widget", G_CALLBACK (add_tasks_toolbar_widget), GTK_BOX(vbox1));
    
    if (!gtk_ui_manager_add_ui_from_string (tasks_uim_widget, ui_info, -1, &error)) {
        g_message ("building toolbar failed: %s", error->message);
        g_error_free (error);
    }
    gtk_ui_manager_ensure_update (tasks_uim_widget);
    ...
    cf_combobox = gtk_combo_box_new_text ();
    gtk_widget_show (cf_combobox);
    gtk_combo_box_set_focus_on_click (GTK_COMBO_BOX (cf_combobox), FALSE);
    GTK_WIDGET_UNSET_FLAGS(cf_combobox, GTK_CAN_FOCUS);
    gtk_table_attach (GTK_TABLE (table), cf_combobox, 1, 2, 0, 1,
                     (GtkAttachOptions) (GTK_EXPAND | GTK_FILL),
                     (GtkAttachOptions) (GTK_FILL), 0, 0);
    g_signal_connect(cf_combobox, "changed", 
                     G_CALLBACK(category_filter_cb), NULL);
    g_signal_connect(G_OBJECT(cf_combobox), "focus", 
                     G_CALLBACK(category_combo_box_focus_cb), NULL);      

    sprintf(tmpbuf, "<i>0000 %s</i>", _("entries"));
    n_items_label = gtk_label_new (tmpbuf);
    gtk_widget_show (n_items_label);
    gtk_widget_set_size_request (n_items_label, 100, -1);
    gtk_table_attach (GTK_TABLE (table), n_items_label, 3, 4, 0, 1,
                     (GtkAttachOptions) (GTK_FILL),
                     (GtkAttachOptions) (0), 0, 0);
    gtk_label_set_use_markup (GTK_LABEL (n_items_label), TRUE);
	...
	tasks_paned = gtk_vpaned_new();
    gtk_widget_show (tasks_paned);
    gtk_paned_set_position(GTK_PANED(tasks_paned), 99999);
	gtk_box_pack_start(GTK_BOX(vbox1), tasks_paned, TRUE, TRUE, 0);
    ...
    gtk_paned_pack1 (GTK_PANED (tasks_paned), top_viewport, FALSE, TRUE);
    ...
    tasks_list_store = gtk_list_store_new(TASKS_NUM_COLUMNS, 
                                         G_TYPE_BOOLEAN,
                                         G_TYPE_STRING, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_STRING,
                                         G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

    tasks_filter = gtk_tree_model_filter_new(GTK_TREE_MODEL(tasks_list_store), NULL);
    gtk_tree_model_filter_set_visible_func (GTK_TREE_MODEL_FILTER(tasks_filter), 
                                            (GtkTreeModelFilterVisibleFunc)tasks_list_filter_cb, 
                                            NULL, NULL);

    tasks_sort = gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(tasks_filter));

    tasks_list = gtk_tree_view_new_with_model(GTK_TREE_MODEL(tasks_sort));
    gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(tasks_list), config.tasks_rules_hint);
    gtk_widget_show (tasks_list);
    GTK_WIDGET_SET_FLAGS (tasks_list, GTK_CAN_DEFAULT);
    gtk_widget_modify_fg(GTK_WIDGET(tasks_list), GTK_STATE_SELECTED, 
                         (& GTK_WIDGET(tasks_list)->style->base[GTK_STATE_SELECTED]));
    gtk_widget_modify_fg(GTK_WIDGET(tasks_list), GTK_STATE_NORMAL, 
                         (& GTK_WIDGET(tasks_list)->style->bg[GTK_STATE_NORMAL]));

	g_signal_connect(G_OBJECT(tasks_list), "button_press_event", 
                     G_CALLBACK(list_dbclick_cb), NULL);      

    tasks_list_selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (tasks_list));  
    g_signal_connect(G_OBJECT(tasks_list_selection), "changed", 
                     G_CALLBACK(tasks_item_selected), NULL);   

    /* create columns */

    renderer = gtk_cell_renderer_toggle_new();
    tasks_columns[COLUMN_DONE] = gtk_tree_view_column_new_with_attributes (_("Done"),
                             renderer,
                             "active", COLUMN_DONE,
                             NULL);
    gtk_tree_view_append_column (GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_DONE]);

    g_signal_connect (renderer, "toggled", G_CALLBACK (done_toggled), 
                      GTK_TREE_MODEL(tasks_list_store));
    
    renderer = gtk_cell_renderer_text_new();
    tasks_columns[COLUMN_DUE_DATE] = gtk_tree_view_column_new_with_attributes(_("Due date"),
                              renderer,
    ...
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_DUE_DATE]);

    renderer = gtk_cell_renderer_text_new();
    tasks_columns[COLUMN_DUE_DATE_JULIAN] = gtk_tree_view_column_new_with_attributes(NULL,
                              renderer,
                              "text", COLUMN_DUE_DATE_JULIAN,
                              "strikethrough", COLUMN_DONE,
                              "foreground", COLUMN_COLOR,
                              NULL);
    gtk_tree_view_column_set_visible (tasks_columns[COLUMN_DUE_DATE_JULIAN], FALSE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_DUE_DATE_JULIAN]);

    renderer = gtk_cell_renderer_text_new();
    tasks_columns[COLUMN_START_DATE_JULIAN] = gtk_tree_view_column_new_with_attributes(NULL,
                              renderer,
                              "text", COLUMN_START_DATE_JULIAN,
                              "strikethrough", COLUMN_DONE,
                              "foreground", COLUMN_COLOR,
                              NULL);
    gtk_tree_view_column_set_visible (tasks_columns[COLUMN_START_DATE_JULIAN], FALSE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_START_DATE_JULIAN]);

    renderer = gtk_cell_renderer_text_new();
    tasks_columns[COLUMN_PRIORITY] = gtk_tree_view_column_new_with_attributes(_("Priority"),
                              renderer,
    ...
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_PRIORITY]);

    renderer = gtk_cell_renderer_text_new();
    tasks_columns[COLUMN_CATEGORY] = gtk_tree_view_column_new_with_attributes(_("Category"),
                              renderer,
    ...
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_CATEGORY]);

    renderer = gtk_cell_renderer_text_new();
    g_object_set(G_OBJECT(renderer), "ellipsize", PANGO_ELLIPSIZE_END, NULL);
    gtk_cell_renderer_set_fixed_size(renderer, 0, -1);
    tasks_columns[COLUMN_SUMMARY] = gtk_tree_view_column_new_with_attributes(_("Summary"),
                              renderer,
                              "text", COLUMN_SUMMARY,
                              "strikethrough", COLUMN_DONE,
                              "foreground", COLUMN_COLOR,
                              NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_SUMMARY]);


    renderer = gtk_cell_renderer_text_new();
    tasks_columns[COLUMN_DESCRIPTION] = gtk_tree_view_column_new_with_attributes(_("Description"),
                              renderer,
                              "text", COLUMN_DESCRIPTION,
                              NULL);
    gtk_tree_view_column_set_visible (tasks_columns[COLUMN_DESCRIPTION], FALSE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_DESCRIPTION]);

    renderer = gtk_cell_renderer_text_new();
    tasks_columns[COLUMN_COLOR] = gtk_tree_view_column_new_with_attributes(NULL,
                              renderer,
                              "text", COLUMN_COLOR,
                              NULL);
    gtk_tree_view_column_set_visible (tasks_columns[COLUMN_COLOR], FALSE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tasks_list), tasks_columns[COLUMN_COLOR]);

    /* configure list options */

    gtk_container_add (GTK_CONTAINER (top_scrolledwindow), tasks_list);
    gtk_tree_view_set_enable_search (GTK_TREE_VIEW(tasks_list), FALSE);

    /* configure sorting */

	gtk_tree_sortable_set_sort_func((GtkTreeSortable *)tasks_sort, 0, 
    ...
    gtk_paned_pack2 (GTK_PANED (tasks_paned), bottom_viewport, TRUE, TRUE);
    ...
    gtk_box_pack_start (GTK_BOX (vbox2), panel_hbox, FALSE, FALSE, 0);
    gtk_widget_show(panel_hbox);

    sprintf(tmpbuf, "%s:", _("Task details"));
    label = gtk_label_new (tmpbuf);
    gtk_widget_show (label);
    gtk_box_pack_start (GTK_BOX (panel_hbox), label, FALSE, FALSE, 0);
    ...
    if(config.enable_tooltips) {
        gtk_tooltips_set_tip (osmo_tooltips, close_button, _("Close description panel"), NULL);
    }
    gtk_box_pack_end (GTK_BOX (panel_hbox), close_button, FALSE, FALSE, 0);
    g_signal_connect (G_OBJECT (close_button), "clicked",
                        G_CALLBACK (panel_close_desc_cb), NULL);

    panel_scrolledwindow = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_show (panel_scrolledwindow);
    gtk_box_pack_start (GTK_BOX (vbox2), panel_scrolledwindow, TRUE, TRUE, 0);
    gtk_scrolled_window_set_shadow_type (GTK_SCROLLED_WINDOW (panel_scrolledwindow), GTK_SHADOW_IN);
    gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (panel_scrolledwindow), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);

    tasks_desc_textview = gtk_text_view_new ();
    gtk_text_view_set_wrap_mode (GTK_TEXT_VIEW (tasks_desc_textview), GTK_WRAP_WORD);
    gtk_text_view_set_pixels_above_lines(GTK_TEXT_VIEW(tasks_desc_textview), 4);
    gtk_text_view_set_left_margin(GTK_TEXT_VIEW(tasks_desc_textview), 4);
    gtk_text_view_set_right_margin(GTK_TEXT_VIEW(tasks_desc_textview), 4);
    gtk_text_view_set_cursor_visible(GTK_TEXT_VIEW(tasks_desc_textview), FALSE);
    gtk_text_view_set_editable(GTK_TEXT_VIEW(tasks_desc_textview), FALSE);
    gtk_widget_show (tasks_desc_textview);
    gtk_container_add (GTK_CONTAINER (panel_scrolledwindow), tasks_desc_textview);

    text_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(tasks_desc_textview));
    gtk_text_buffer_create_tag (text_buffer, "italic",
                  "style", PANGO_STYLE_ITALIC, NULL);
    font_tag_object = gtk_text_buffer_create_tag (text_buffer, "info_font",
