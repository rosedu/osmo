...
void
insert_date_cb (GtkWidget *widget, gpointer data) {
...
gchar tmpbuf[BUFFER_SIZE];
    ...
    date_from.year = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_start_year));
    date_from.month = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_start_month)) - 1;
    date_from.day = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_start_day));
    time_from.hour = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_start_hour));
    time_from.minute = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_start_minute));
    time_from.second = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_start_second));
    days = (gint)get_number_of_days_in_month (date_from.month, date_from.year);
    if (date_from.day > days) {
        date_from.day = days;
    }

    date_to.year = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_end_year));
    date_to.month = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_end_month)) - 1;
    date_to.day = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_end_day));
    time_to.hour = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_end_hour));
    time_to.minute = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_end_minute));
    time_to.second = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton_end_second));
    days = (gint)get_number_of_days_in_month (date_to.month, date_to.year);
    if (date_to.day > days) {
        date_to.day = days;
    }

    days = calculate_date_time_diff (date_from, time_from, date_to, time_to,
                                     &date_diff, &time_diff);

    if (days == -1) {
        sprintf (tmpbuf, "<big>%s</big>", _("Start date is after the end date!"));
        gtk_label_set_markup (GTK_LABEL (appGUI->cal->label_result_1), tmpbuf);
        gtk_label_set_markup (GTK_LABEL (appGUI->cal->label_result_1_2), "<big> </big>");
        gtk_label_set_markup (GTK_LABEL (appGUI->cal->label_result_2), "<small> </small>");
        gtk_label_set_markup (GTK_LABEL (appGUI->cal->label_a), "<small>\n </small>");
        gtk_label_set_markup (GTK_LABEL (appGUI->cal->label_result_3), "<small> \n \n \n </small>");
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_1), FALSE);
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_1_2), FALSE);
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_2), FALSE);
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_3), FALSE);
    } else {
        sprintf (tmpbuf, "<big>%d %s, %d %s, %d %s</big>",
        ...
        gtk_label_set_markup (GTK_LABEL (appGUI->cal->label_a), tmpbuf);

        sprintf (tmpbuf, "<small>(%s: %d %s, %d %s, %d %s, %d %s)</small>",
            _("or"),
        ...
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_1), TRUE);
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_1_2), TRUE);
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_2), TRUE);
        gtk_label_set_selectable (GTK_LABEL (appGUI->cal->label_result_3), TRUE);
    }
}
...
gchar tmpbuf[BUFFER_SIZE];
    ...
    year = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_start_year));
    month = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_start_month)) - 1;
    day = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_start_day));
    hour = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_start_hour));
    minute = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_start_minute));
    second = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_start_second));

    year2 = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_end_year));
    month2 = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_end_month));
    week2 = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_end_week));
    day2 = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_end_day));
    hour2 = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_end_hour));
    minute2 = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_end_minute));
    second2 = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON
                (appGUI->cal->spinbutton2_end_second));
    ...
    /* add */
        ...
        n = (gint)get_number_of_days_in_month (month, year);
        if (day > n) {
            day = n;
        }
    ...
    if (year < 1) {
        gtk_label_set_text (GTK_LABEL (appGUI->cal->label2_result), 
            _("This calculator only supports dates after year 1."));

    } else {
        if (hour == 0 && minute == 0 && second == 0) {
            sprintf (tmpbuf, "<big>%s</big>", julian_to_str (julian, DATE_FULL));
        } else {
            sprintf (timebuf, "%02d:%02d.%02d", hour, minute, second);
            sprintf (tmpbuf, "<big>%s, %s</big>", timebuf, julian_to_str (julian, DATE_FULL));
        }
...
char      tmpbuf[BUFFER_SIZE];
    ...
    sprintf (tmpbuf, "<b>%s:</b>", _("Start date and time"));
    ...
    sprintf (tmpbuf, "<b>%s:</b>", _("End date and time"));
    label = gtk_label_new (tmpbuf);
    gtk_widget_show (label);
    gtk_frame_set_label_widget (GTK_FRAME (frame), label);
    gtk_label_set_use_markup (GTK_LABEL (label), TRUE);
    ...
    sprintf (tmpbuf, "<small>\n<i>%s:</i></small>", _("Alternative time units"));
    appGUI->cal->label_a = gtk_label_new (tmpbuf);
    gtk_widget_show (appGUI->cal->label_a);
    gtk_box_pack_start (GTK_BOX (vbox_result), appGUI->cal->label_a, FALSE, FALSE, 0);
    gtk_label_set_use_markup (GTK_LABEL (appGUI->cal->label_a), TRUE);
    gtk_misc_set_alignment (GTK_MISC (appGUI->cal->label_a), 0, 0.5);
    ...
    sprintf (tmpbuf, "<b>%s:</b>", _("Date and time to add or substract from"));
    label = gtk_label_new (tmpbuf);
    gtk_widget_show (label);
    gtk_frame_set_label_widget (GTK_FRAME (frame), label);
    gtk_label_set_use_markup (GTK_LABEL (label), TRUE);
    ...
    radiobutton_sub = gtk_radio_button_new_with_mnemonic (NULL, _("substract"));
    ...
    sprintf (tmpbuf, "<b>%s:</b>", _("Time to add or substract"));
    label = gtk_label_new (tmpbuf);
    gtk_widget_show (label);
    gtk_frame_set_label_widget (GTK_FRAME (frame), label);
    gtk_label_set_use_markup (GTK_LABEL (label), TRUE);
