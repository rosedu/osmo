    ...
    if (g_access (filename, R_OK) != -1) {
        ...
        if (g_file_get_contents (filename, &appGUI->cnt->file_buffer, &appGUI->cnt->file_length, NULL) == TRUE) {
            ...
            if (n_records) {
                ...
                gtk_window_set_default_size (GTK_WINDOW(appGUI->cnt->import_window), IMPORT_WINDOW_SIZE_X, IMPORT_WINDOW_SIZE_Y);
                gtk_window_set_position(GTK_WINDOW(appGUI->cnt->import_window), GTK_WIN_POS_CENTER_ON_PARENT);
                ...
                gtk_window_get_position (GTK_WINDOW(appGUI->cnt->import_window), &win_xpos, &win_ypos);
                gtk_window_move (GTK_WINDOW (appGUI->cnt->import_window), win_xpos-IMPORT_WINDOW_SIZE_X/2, win_ypos-10);
