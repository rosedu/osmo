...
check_date(guint year, guint mm, guint dd) {
    ...
    if ((dd < 1) || (dd > month_length[is_leap_year(year)][mm])) return FALSE;
    return TRUE;
}
...
calc_days(guint year, guint mm, guint dd) {
    ...
    if ((dd < 1) || (dd > month_length[(lp = is_leap_year(year))][mm])) return(0L);
...
gui_calendar_init (GuiCalendar *calendar) {
    ...
    calendar->mark_sign = g_utf8_get_char("·");
...
calendar_set_month_next (GuiCalendar *calendar) {
    ...
    month_len = month_length[is_leap_year (calendar->year)][calendar->month + 1];
...
calendar_compute_days (GuiCalendar *calendar) {
    ...
    ndays_in_month = month_length[is_leap_year (year)][month];
    ...
    if (month > 1)
        ndays_in_prev_month = month_length[is_leap_year (year)][month-1];
    else
        ndays_in_prev_month = month_length[is_leap_year (year)][12];
...
calendar_set_month_prev (GuiCalendar *calendar) {
    ...
    if (month_len < calendar->selected_day) {
        ...
        if (calendar->selected_day < 0)
            calendar->selected_day = calendar->selected_day + 1 + month_length[is_leap_year (calendar->year)][calendar->month + 1];
        gui_calendar_select_day (calendar, calendar->selected_day);
    }
...
gui_calendar_size_request (GtkWidget      *widget,
    ...
    if (calendar->display_flags & GUI_CALENDAR_SHOW_HEADING) {
        ...
        pango_layout_set_text (layout, "2000", -1);
        pango_layout_get_pixel_extents (layout, NULL, &logical_rect);
...
calendar_paint_day (GuiCalendar *calendar,
    ...
    if (calendar->mark_sign) {
        if (calendar->marked_date[day-1] && calendar->day_month[row][col] == MONTH_CURRENT) {
            g_snprintf (buffer, sizeof (buffer), "%d%C", day, calendar->mark_sign);
        } else {
    ...
    if (calendar->tc_marked_date[day-1] && calendar->day_month[row][col] == MONTH_CURRENT) {
        gdk_cairo_set_source_color (cr, &calendar->tc_mark_color);
        cairo_arc (cr, day_rect.x+day_rect.width/2.0, day_rect.y+day_rect.height/2.0, 
                   day_rect.height/2.0-3.0, 0.0, 2.0 * M_PI);
        cairo_fill (cr);
    }
    ...
    if (calendar->day_month[row][col] == MONTH_PREV) {
        ...
        if (calendar->selected_day == day) {
            ...
            if (calendar->cursor_type == CURSOR_FRAME) {
                ...
                gdk_cairo_set_source_color (cr, &calendar->selector_color);
        ...
        if (calendar->cursor_type == CURSOR_BLOCK && calendar->selected_day == day) {
...
void
gui_calendar_set_mark_symbol (GuiCalendar *calendar, const gchar *symbol) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
...
void
gui_calendar_set_tc_mark_color (GuiCalendar *calendar, const gchar *color) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    gdk_color_parse(color, &calendar->tc_mark_color);
}
