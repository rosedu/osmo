...
gchar *contact_fields_tags_name[] = {
    ...
    /*--------------------------------------------------*/
    N_("Home address"), "home_address", N_("Home postcode"), "home_postcode", N_("Home city"), "home_city", N_("Home state"), 
    "home_state", N_("Home country"), "home_country",
    /*--------------------------------------------------*/

    N_("Organization"), "organization", N_("Department"), "department", 
    
    /*--------------------------------------------------*/
    N_("Work address"), "work_address", N_("Work postcode"), "work_postcode", N_("Work city"), "work_city", N_("Work state"), 
    "work_state", N_("Work country"), "work_country", 
    /*--------------------------------------------------*/
    ...
    N_("E-Mail"), "email_1", N_("E-Mail 2"), "email_2", N_("E-Mail 3"), "email_3", N_("E-Mail 4"), "email_4",
    N_("WWW"), "www_1", N_("WWW 2"), "www_2", N_("WWW 3"), "www_3", N_("WWW 4"), "www_4",
    /*--------------------------------------------------*/
...
gboolean state;
    ...
    n = 0;
    while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(contacts_filter), &iter, NULL, n++));
    state = (n == 1) ? FALSE : TRUE;
    gtk_widget_set_sensitive(gtk_ui_manager_get_widget (contacts_uim_widget, "/toolbar/export"), state);
}

/*------------------------------------------------------------------------------*/

void
show_contacts_desc_panel (gboolean enable) {
...
GtkAdjustment *adj;
    ...
    if(enable == TRUE) {
        ...
        if (gtk_tree_selection_get_selected (contacts_list_selection, &model, &iter)) {
            ...
            gtk_paned_set_position(GTK_PANED(contacts_paned), config.contacts_pane_pos);
            ...
            if (visible_path) {
                ...
                gtk_tree_view_get_cell_area (GTK_TREE_VIEW (contacts_list), visible_path, NULL, &rect);
                gtk_tree_view_get_visible_rect (GTK_TREE_VIEW (contacts_list), &visible_rect);

                if (rect.y < visible_rect.y || rect.y > visible_rect.y + visible_rect.height) {
                        gtk_tree_view_scroll_to_cell (GTK_TREE_VIEW (contacts_list), visible_path, NULL, TRUE, 0.5, 0.0);
                }
        ...
        config.contacts_pane_pos = gtk_paned_get_position(GTK_PANED(contacts_paned));
        gtk_paned_set_position(GTK_PANED(contacts_paned), 99999);

        adj = gtk_scrolled_window_get_vadjustment (GTK_SCROLLED_WINDOW(contacts_panel_scrolledwindow));
        gtk_adjustment_set_value (adj, 0.0);
        gtk_adjustment_value_changed (adj);

    }

    contacts_panel_status = enable;
}
...
guint32 date;
    ...
    if (contacts_filter_disabled == TRUE) {
        return TRUE;
    }

    text = gtk_entry_get_text(GTK_ENTRY(contacts_find_entry));
...
void
contacts_select_first_position_in_list(void) {

GtkTreeIter     iter;
GtkTreePath     *path;

    /* set cursor at first position */
    if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(contacts_list_store), &iter) == TRUE) {
...
guint32 date;
    ...
    if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
        ...
        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (contacts_uim_widget, "/toolbar/edit"), TRUE);
        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (contacts_uim_widget, "/toolbar/delete"), TRUE);
        set_export_active ();

        text_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(contacts_desc_textview));
        gui_clear_text_buffer (text_buffer, &titer);

        if (photo_image != NULL) {      /* remove old photo */
            gtk_widget_destroy(photo_image);
            photo_image = NULL;
        }
            ...
            if (i == COLUMN_BIRTH_DAY_DATE || i == COLUMN_NAME_DAY_DATE) {
                ...
                if (i != COLUMN_PHOTO) {
                    ...
                    if (i == COLUMN_FIRST_NAME || i == COLUMN_SECOND_NAME || i == COLUMN_LAST_NAME) {
                        ...
                        if ((i >= COLUMN_HOME_PHONE_1 && i <= COLUMN_HOME_PHONE_4)  ||
                            ...
                            if ((activate_home_phone == FALSE && i == COLUMN_HOME_PHONE_1) ||
                               ...
                               (activate_www == FALSE && i == COLUMN_WWW_1)) {
                                ...
                                sprintf(tmpbuf, "%s: ", gettext(contact_fields_tags_name[2*i]));
                            } else {
                                sprintf(tmpbuf, " ");
                            }
                        ...
                        if (i != COLUMN_INFO) {
                            space_line[longest_tag_name - strlen(tmpbuf) + 2] = '\0';
                        ...
                        if (i == COLUMN_BLOG || (i >= COLUMN_EMAIL_1 && i <= COLUMN_EMAIL_4) || (i >= COLUMN_WWW_1 && i <= COLUMN_WWW_4)) { /* insert link */
                            gui_url_insert_link(&contacts_links_list, &contacts_link_index, contacts_desc_textview , &titer, config.contact_link_color, config.tags_font_size, text);
                        } else {
        ...
        if (text != NULL) {
            ...
            insert_photo(photo_full_path, &titer);
            g_free(text);
        }

    } else {
        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (contacts_uim_widget, "/toolbar/edit"), FALSE);
        gtk_widget_set_sensitive(gtk_ui_manager_get_widget (contacts_uim_widget, "/toolbar/delete"), FALSE);
        set_export_active ();
    }
}
...
gint x, y;
    ...
    if (photo_image != NULL) {
        gtk_text_view_buffer_to_window_coords(GTK_TEXT_VIEW(contacts_desc_textview), GTK_TEXT_WINDOW_TEXT,
                                              contacts_desc_textview->allocation.width-config.photo_width-5, 5, &x, &y);
        gtk_text_view_move_child(GTK_TEXT_VIEW(contacts_desc_textview), photo_image,
                                 contacts_desc_textview->allocation.width-config.photo_width-5, y);
    }
}

void
contacts_desc_size_allocate (GtkWidget *widget, GtkAllocation *allocation, gpointer data) {
    contacts_desc_scrolled(NULL, NULL);
}

/*------------------------------------------------------------------------------*/

void
add_contacts_toolbar_widget (GtkUIManager *contacts_uim_widget, GtkWidget *widget, GtkBox *box) {

GtkWidget *handle_box;

    if (GTK_IS_TOOLBAR (widget)) {

        contacts_toolbar = GTK_TOOLBAR (widget);
...
gint i;
    ...
    if(strlen(gtk_entry_get_text (GTK_ENTRY(contacts_find_entry)))) {
        ...
        if (config.show_after_search == TRUE) {
            ...
            i = 0;
            while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(contacts_filter), &iter, NULL, i++));
            ...
            if (i-1 != 0) {
                ...
                gui_clear_text_buffer (gtk_text_view_get_buffer(GTK_TEXT_VIEW(contacts_desc_textview)), &titer);
            }
        }
    }
    return FALSE;
...
contacts_list_dbclick_cb(GtkWidget * widget, GdkEventButton * event, gpointer func_data) {
    ...
    if ((event->type==GDK_2BUTTON_PRESS) && (event->button == 1)) {
        ...
        if (contacts_panel_status == TRUE) {
            contacts_edit_item_cb (NULL, NULL);
        } else {
            show_contacts_desc_panel(TRUE);
        }
        return TRUE;
...
void
gui_create_contacts(GtkWidget *notebook) {
...
guint n_entries = G_N_ELEMENTS (entries);
    ...
    for(i=0; i < CONTACTS_NUM_COLUMNS; i++) {
        if (longest_tag_name < strlen(gettext(contact_fields_tags_name[i*2]))) {
            longest_tag_name = strlen(gettext(contact_fields_tags_name[i*2]));
        }
    }
    ...
    contacts_uim_widget = gtk_ui_manager_new ();

    gtk_ui_manager_insert_action_group (contacts_uim_widget, action_group, 0);
    g_signal_connect (contacts_uim_widget, "add_widget", G_CALLBACK (add_contacts_toolbar_widget), GTK_BOX(vbox1));

    if (!gtk_ui_manager_add_ui_from_string (contacts_uim_widget, ui_info, -1, &error)) {
        g_message ("building toolbar failed: %s", error->message);
        g_error_free (error);
    }
    gtk_ui_manager_ensure_update (contacts_uim_widget);

    gtk_toolbar_set_style (contacts_toolbar, GTK_TOOLBAR_ICONS);
    ...
    contacts_find_entry = gtk_entry_new();
    gtk_entry_set_max_length(GTK_ENTRY(contacts_find_entry), 128);
    gtk_widget_show (contacts_find_entry);
    g_signal_connect (G_OBJECT(contacts_find_entry), "key_release_event",
                        G_CALLBACK(contacts_search_entry_changed_cb), NULL);
    gtk_box_pack_start (GTK_BOX (hbox2), contacts_find_entry, TRUE, TRUE, 0);

    contacts_find_combobox = gtk_combo_box_new_text ();
    gtk_widget_show (contacts_find_combobox);
    gtk_combo_box_set_focus_on_click (GTK_COMBO_BOX (contacts_find_combobox), FALSE);
    gtk_box_pack_start (GTK_BOX (hbox2), contacts_find_combobox, FALSE, FALSE, 0);
    g_signal_connect (G_OBJECT (contacts_find_combobox), "changed",
                      G_CALLBACK (contacts_find_type_selected_cb), NULL);
    g_signal_connect(G_OBJECT(contacts_find_combobox), "focus",
                     G_CALLBACK(find_combo_box_focus_cb), NULL);
    gtk_combo_box_append_text (GTK_COMBO_BOX (contacts_find_combobox), _("First Name"));
    gtk_combo_box_append_text (GTK_COMBO_BOX (contacts_find_combobox), _("Last Name"));
    gtk_combo_box_append_text (GTK_COMBO_BOX (contacts_find_combobox), _("All fields"));
...
/******************************************************************************************/
    ...
    contacts_paned = gtk_vpaned_new();
    gtk_widget_show (contacts_paned);
    gtk_paned_set_position(GTK_PANED(contacts_paned), 99999);
    gtk_box_pack_start(GTK_BOX(vbox1), contacts_paned, TRUE, TRUE, 0);
    ...
    gtk_paned_pack1 (GTK_PANED (contacts_paned), top_viewport, FALSE, TRUE);
    ...
    contacts_list_store = gtk_list_store_newv(CONTACTS_NUM_COLUMNS, &contact_columns_types[0]);

    contacts_filter = gtk_tree_model_filter_new(GTK_TREE_MODEL(contacts_list_store), NULL);
    gtk_tree_model_filter_set_visible_func (GTK_TREE_MODEL_FILTER(contacts_filter),
                                            (GtkTreeModelFilterVisibleFunc)contacts_list_filter_cb,
                                            NULL, NULL);

    contacts_sort = gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(contacts_filter));

    contacts_list = gtk_tree_view_new_with_model(GTK_TREE_MODEL(contacts_sort));
    gtk_tree_view_set_rules_hint(GTK_TREE_VIEW(contacts_list), config.contacts_rules_hint);
    gtk_widget_show (contacts_list);
    GTK_WIDGET_SET_FLAGS (contacts_list, GTK_CAN_DEFAULT);
    gtk_widget_modify_fg(GTK_WIDGET(contacts_list), GTK_STATE_SELECTED,
                         (& GTK_WIDGET(contacts_list)->style->base[GTK_STATE_SELECTED]));

    g_signal_connect(G_OBJECT(contacts_list), "button_press_event",
                     G_CALLBACK(contacts_list_dbclick_cb), NULL);

    contacts_list_selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (contacts_list));
    g_signal_connect(G_OBJECT(contacts_list_selection), "changed",
                     G_CALLBACK(contacts_item_selected_cb), NULL);

    /* columns setup */

    for (i = COLUMN_GROUP; i < CONTACTS_NUM_COLUMNS; i++) {
        renderer[i] = gtk_cell_renderer_text_new();
        contacts_columns[i] = gtk_tree_view_column_new_with_attributes(gettext(contact_fields_tags_name[i*2]),
                                                              renderer[i], "text", i, NULL);
        gtk_tree_view_append_column (GTK_TREE_VIEW(contacts_list), contacts_columns[i]);

        if(i != COLUMN_FIRST_NAME && i != COLUMN_LAST_NAME && i != COLUMN_GROUP) {
            gtk_tree_view_column_set_visible (contacts_columns[i], FALSE);
        } else {
            if (config.hide_group_column == TRUE && i == COLUMN_GROUP) {
                gtk_tree_view_column_set_visible (contacts_columns[i], FALSE);
            }
            gtk_tree_view_column_set_expand (contacts_columns[i], TRUE);
        }
    }

    gtk_container_add (GTK_CONTAINER (top_scrolledwindow), contacts_list);
    gtk_tree_view_set_enable_search (GTK_TREE_VIEW(contacts_list), FALSE);

    /* configure sorting */

    gtk_tree_sortable_set_sort_func((GtkTreeSortable *)contacts_sort, 0,
                                    (GtkTreeIterCompareFunc)custom_contacts_sort_function, NULL, NULL);
    ...
    gtk_paned_pack2 (GTK_PANED (contacts_paned), bottom_viewport, TRUE, TRUE);
    ...
    if(config.enable_tooltips) {
        gtk_tooltips_set_tip (osmo_tooltips, close_button, _("Close contact panel"), NULL);
    }
    gtk_box_pack_end (GTK_BOX (panel_hbox), close_button, FALSE, FALSE, 0);
    g_signal_connect (G_OBJECT (close_button), "clicked",
                        G_CALLBACK (contacts_panel_close_desc_cb), NULL);

    contacts_panel_scrolledwindow = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_show (contacts_panel_scrolledwindow);
    gtk_box_pack_start (GTK_BOX (vbox2), contacts_panel_scrolledwindow, TRUE, TRUE, 0);
    gtk_scrolled_window_set_shadow_type (GTK_SCROLLED_WINDOW (contacts_panel_scrolledwindow), GTK_SHADOW_IN);
    gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (contacts_panel_scrolledwindow), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);

    contacts_desc_textview = gtk_text_view_new ();
    gtk_text_view_set_wrap_mode (GTK_TEXT_VIEW (contacts_desc_textview), GTK_WRAP_WORD);
    gtk_text_view_set_pixels_above_lines(GTK_TEXT_VIEW(contacts_desc_textview), 4);
    gtk_text_view_set_left_margin(GTK_TEXT_VIEW(contacts_desc_textview), 4);
    gtk_text_view_set_right_margin(GTK_TEXT_VIEW(contacts_desc_textview), 4);
    gtk_text_view_set_cursor_visible(GTK_TEXT_VIEW(contacts_desc_textview), FALSE);
    gtk_text_view_set_editable(GTK_TEXT_VIEW(contacts_desc_textview), FALSE);
    gtk_widget_show (contacts_desc_textview);
    gtk_container_add (GTK_CONTAINER (contacts_panel_scrolledwindow), contacts_desc_textview);

    gui_url_setup(&contacts_links_list, &contacts_link_index, contacts_desc_textview);

    adjustment = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(contacts_panel_scrolledwindow));
    g_signal_connect(G_OBJECT(adjustment), "value-changed", G_CALLBACK(contacts_desc_scrolled), NULL);
    g_signal_connect(G_OBJECT(contacts_desc_textview), "size_allocate", G_CALLBACK(contacts_desc_size_allocate), NULL);

    text_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(contacts_desc_textview));

    contact_name_tag_object = gtk_text_buffer_create_tag (text_buffer, "big",
                                                          "size", config.name_font_size * PANGO_SCALE, NULL);
    contact_tags_value_tag_object = gtk_text_buffer_create_tag (text_buffer, "normal",
                                                                "size", config.tags_font_size * PANGO_SCALE, NULL);
    contact_color_tag_object = gtk_text_buffer_create_tag (text_buffer, "tag_name",
    ...
    gtk_combo_box_set_active (GTK_COMBO_BOX (contacts_find_combobox), config.find_mode);

    contacts_filter_disabled = FALSE;
    gtk_tree_sortable_sort_column_changed((GtkTreeSortable *)contacts_sort);
    gtk_tree_model_filter_refilter(GTK_TREE_MODEL_FILTER(contacts_filter));
}
...
void
insert_photo(gchar *photo_filename, GtkTextIter * iter) {
...
gint rowstride, channels, bc1, bc2, bc3, bc4;
        ...
        buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(contacts_desc_textview));
        ...
        if (pixbuf != NULL) {
                ...
                photo_image = gtk_image_new_from_pixbuf(pixbuf);

                if (photo_image != NULL) {
                    gtk_text_view_add_child_in_window(GTK_TEXT_VIEW(contacts_desc_textview), photo_image, GTK_TEXT_WINDOW_TEXT, 
                                                      contacts_desc_textview->allocation.width-d_photo_width-5, 5);
                    gtk_widget_show_all(contacts_desc_textview);
                }
