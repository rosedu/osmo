...
set_contacts_columns_width (GUI *appGUI) {
	...
	if (gtk_tree_view_column_get_visible(col) == TRUE) {
	...
	if (gtk_tree_view_column_get_visible(col) == TRUE) {
		gtk_tree_view_column_set_fixed_width (col, config.contacts_column_idx_2_width - 
											  2 * utl_gui_get_sw_vscrollbar_width (appGUI->cnt->scrolled_win));
	}
}
...
/******************************************************************************************/
    ...
    gtk_container_add (GTK_CONTAINER (top_viewport), appGUI->cnt->scrolled_win);
