...
birthdays_window_close_cb (GtkWidget *widget, GdkEvent *event, gpointer user_data) {
    ...
    /*gtk_window_get_size (GTK_WINDOW(appGUI->cnt->birthdays_window),*/
                        /*&config.birthdays_window_size_x, &config.birthdays_window_size_y);*/
...
guint c_day, c_month, c_year;
    ...
    gtk_window_set_default_size (GTK_WINDOW(appGUI->cnt->birthdays_window), 400, 550);
    ...
    appGUI->cnt->birthdays_list_store = gtk_list_store_new(BIRTHDAYS_NUM_COLUMNS, G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT);
    ...
    while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(appGUI->cnt->contacts_list_store), &iter, NULL, i++)) {
        ...
        if (date != 0) {
            ...
            gtk_list_store_set(appGUI->cnt->birthdays_list_store, &n_iter, B_COLUMN_NAME, buffer, -1);
            ...
            if (cdate_birthday != NULL) {
                if (g_date_valid (cdate_birthday)) {
                    ...
                    if ((g_date_get_month (cdate_birthday) < get_current_month ()) || 
                       (g_date_get_month (cdate_birthday) == get_current_month () &&
