...
gboolean flag;
	...
	if (check_contacts (julian, appGUI) == TRUE) {
		g_strlcat (desc, "\n", BUFFER_SIZE);
	...
	if (check_tasks (julian, FALSE, appGUI) == TRUE) {
		if (flag == FALSE) g_strlcat (desc, "\n", BUFFER_SIZE);
		g_snprintf (tmpbuf, BUFFER_SIZE, "\n%s", _("Task found"));
		g_strlcat (desc, tmpbuf, BUFFER_SIZE);
		flag = TRUE;
	}
...
static void add_credits_section(GUI *appGUI, const gchar *section_title,
	OsmoCreditEntry *credits, guint32 n_credits)
{
	...
	if(init) {
		init = FALSE;
		gtk_text_buffer_create_tag(appGUI->about_entry_buffer, "fixed",
	...
	for(i = 0; i < n_credits; i ++) {
		...
		gtk_text_buffer_insert(appGUI->about_entry_buffer,
			&appGUI->about_entry_iter, s, -1);
		g_free(s);

		gui_url_insert_link(&appGUI->about_links_list,
			&appGUI->about_link_index, appGUI->about_textview,
			&appGUI->about_entry_iter, NULL, NULL, credits[i].email, FALSE,
			appGUI);

		gtk_text_buffer_insert(appGUI->about_entry_buffer,
			&appGUI->about_entry_iter, ">\n", -1);
	}
}
...
char tmpbuf[BUFFER_SIZE];
    ...
    if (appGUI->tiny_gui == TRUE) {
        appGUI->about_radiobutton = gui_stock_label_radio_button(NULL, OSMO_STOCK_ABOUT, GTK_ICON_SIZE_LARGE_TOOLBAR);
    } else {
    ...
    if (appGUI->tiny_gui == TRUE) {
        appGUI->help_radiobutton = gui_stock_label_radio_button(NULL, OSMO_STOCK_HELP, GTK_ICON_SIZE_LARGE_TOOLBAR);
    } else {
    ...
    if (appGUI->tiny_gui == TRUE) {
        appGUI->license_radiobutton = gui_stock_label_radio_button(NULL, OSMO_STOCK_LICENSE, GTK_ICON_SIZE_LARGE_TOOLBAR);
    } else {
