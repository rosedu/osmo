...
gboolean flag;
    ...
    current_filename = g_strdup (notes_get_filename (appGUI));
                ...
                while (node != NULL) {
                    ...
                    if(!xmlStrcmp(node->name, (xmlChar *) "entry")) {
                        ...
                        while (cnode != NULL) {
                            ...
                            if ((!xmlStrcmp(cnode->name, (const xmlChar *) "editor_line"))) {
                                key = xmlNodeListGetString(doc, cnode->xmlChildrenNode, 1);
                                if (key != NULL) {
                                    sscanf((gchar *) key, "%d", &editor_line);
                                    xmlFree(key);
                                }
                            }
...
guint32 last_changes_date_julian, create_date_julian;
    ...
    while (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(appGUI->nte->notes_list_store), &iter, NULL, i++)) {
        ...
        sprintf(temp, "%d", (guint32) editor_line);
        xmlNewChild(note_node, NULL, (const xmlChar *) "editor_line", (xmlChar *) temp);
