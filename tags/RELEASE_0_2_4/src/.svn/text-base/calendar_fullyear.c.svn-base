...
{
	GUI *appGUI = (GUI *) user_data;
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (appGUI->cal->fy_spinbutton), utl_get_current_year ());
}
...
guint       month, year;
	...
	fy_spinbutton_adj = gtk_adjustment_new (utl_get_current_year (), JULIAN_GREGORIAN_YEAR, 9999, 1, 10, 10);
	...
	display_calendar (utl_get_current_year (), appGUI);
	gtk_widget_set_sensitive (appGUI->cal->cyear_button, FALSE);
