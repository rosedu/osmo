...
const gchar *text;
    ...
    if(strlen(text) && i != COLUMN_HOME_PHONE_4 && i != COLUMN_WORK_PHONE_4 &&
       i != COLUMN_CELL_PHONE_4 && i != COLUMN_EMAIL_4 && i != COLUMN_WWW_4) {
        gtk_widget_set_sensitive (msg->appGUI->cnt->next_field_button[i], FALSE);
        gtk_widget_show (msg->appGUI->cnt->contact_entries[i+1]);
...
static MESSAGE msg_selected;
    ...
    gui_calendar_set_selector_alpha (GUI_CALENDAR (msg->appGUI->cnt->select_date_calendar), 
                                             config.selector_alpha);
    gui_calendar_set_today_marker_type (GUI_CALENDAR (msg->appGUI->cnt->select_date_calendar), 
                                             config.today_marker_type);
    gui_calendar_set_today_marker_color (GUI_CALENDAR (msg->appGUI->cnt->select_date_calendar), 
                                             config.mark_current_day_color);
    gui_calendar_set_today_marker_alpha (GUI_CALENDAR (msg->appGUI->cnt->select_date_calendar), 
                                             config.mark_current_day_alpha);
...
gchar *contact_replace_tags[] = {
    ...
    gtk_table_attach (GTK_TABLE (table), appGUI->cnt->combobox_group, 2, 4, pos, pos+1,
                      (GtkAttachOptions) (GTK_FILL),
                      (GtkAttachOptions) (GTK_FILL), 0, 0);
    gui_create_category_combobox (GTK_COMBO_BOX(appGUI->cnt->combobox_group), appGUI->opt->contacts_group_store, TRUE);
    ...
    for(i=COLUMN_FIRST_NAME; i<COLUMN_BIRTH_DAY_DATE; i++) {
        ...
        if (i == COLUMN_FIRST_NAME) {
            appGUI->cnt->next_field_button[i] = gui_stock_button (GTK_STOCK_GO_DOWN, FALSE);
...
xmlChar *escaped;
    ...
    if ((appGUI->save_status & WRT_CONTACTS) != 0) return;
        ...
        for (j = COLUMN_GROUP; j < CONTACTS_NUM_COLUMNS; j++) {
            ...
            if (j != COLUMN_ID) {
                ...
                if (j == COLUMN_BIRTH_DAY_DATE || j == COLUMN_NAME_DAY_DATE) {
                    ...
                    if (item != NULL) {
                        if (strlen(item)) {
							escaped = xmlEncodeEntitiesReentrant(doc,
								(const xmlChar *) item);
