...
gchar tmpbuf[BUFFER_SIZE];
    ...
    if(appGUI->tsk->tasks_edit_state == TRUE) {
        ...
        item->start_date_julian = utl_get_current_julian ();
    }
...
void
day_selected_cb (GuiCalendar *calendar, gpointer user_data)
{
...
TIME tm;
	...
	gui_calendar_get_date (calendar, &year, &month, &day);
	appGUI->tsk->tasks_due_julian_day = utl_dmy_to_julian (day, month + 1, year);
...
{
	GUI *appGUI = (GUI *) user_data;
    gui_update_command_status (appGUI->tsk->alarm_cmd_entry, appGUI->tsk->alarm_cmd_valid_image, appGUI);
			...
			if (!xmlStrcmp (main_node->name, (xmlChar *) TASKS_ENTRIES_NAME)) {
				...
				while (node != NULL) {
					...
					if (!xmlStrcmp (node->name, (xmlChar *) "entry")) {
						...
						while (cnode != NULL) {
							utl_xml_get_int ("status", &(item->done), cnode, doc);
							utl_xml_get_uint ("due_date", &(item->due_date_julian), cnode, doc);
							utl_xml_get_int ("due_time", &(item->due_time), cnode, doc);
							utl_xml_get_uint ("start_date", &(item->start_date_julian), cnode, doc);
							utl_xml_get_uint ("done_date", &(item->done_date_julian), cnode, doc);
							utl_xml_get_int ("active", &(item->active), cnode, doc);
							utl_xml_get_int ("active_warning", &(item->active_warning), cnode, doc);
							utl_xml_get_int ("offline_ignore", &(item->offline_ignore), cnode, doc);
							utl_xml_get_int ("repeat", &(item->repeat), cnode, doc);
							utl_xml_get_int ("repeat_day", &(item->repeat_day), cnode, doc);
							utl_xml_get_int ("repeat_month_interval", &(item->repeat_month_interval), cnode, doc);
							utl_xml_get_int ("repeat_day_interval", &(item->repeat_day_interval), cnode, doc);
							utl_xml_get_int ("repeat_start_day", &(item->repeat_start_day), cnode, doc);
							utl_xml_get_int ("repeat_time_start", &(item->repeat_time_start), cnode, doc);
							utl_xml_get_int ("repeat_time_end", &(item->repeat_time_end), cnode, doc);
							utl_xml_get_int ("repeat_time_interval", &(item->repeat_time_interval), cnode, doc);
							utl_xml_get_int ("repeat_counter", &(item->repeat_counter), cnode, doc);
							utl_xml_get_int ("warning_days", &(item->warning_days), cnode, doc);
							utl_xml_get_int ("warning_time", &(item->warning_time), cnode, doc);
							utl_xml_get_int ("priority", &priority_n, cnode, doc);
							utl_xml_get_str ("alarm_command", &(item->alarm_command), cnode, doc);
							utl_xml_get_str ("category", &(item->category), cnode, doc);
							utl_xml_get_str ("summary", &(item->summary), cnode, doc);
							utl_xml_get_str ("description", &(item->desc), cnode, doc);
							cnode = cnode->next;
						}
...
gint i;
	...
	while (gtk_tree_model_iter_nth_child (GTK_TREE_MODEL (appGUI->tsk->tasks_list_store), &iter, NULL, i++)) {
		...
		if (item != NULL) {
			...
			utl_xml_put_int ("active_warning", item->active_warning, note_node);
			...
			utl_xml_put_int ("priority", get_priority_index (gettext (item->priority)), note_node);
