...
gchar tmpbuf[BUFFER_SIZE];
    ...
    response = gui_create_dialog(GTK_MESSAGE_QUESTION, tmpbuf, GTK_WINDOW(appGUI->cal->ical_export_window));
...
gchar tmpbuf[BUFFER_SIZE];
    ...
    if (gui_check_overwrite_file (filename, appGUI->cal->ical_export_window, appGUI) != 0) {
        return;
    } else {
        g_unlink (filename);
    }
    ...
    if (i >= 2) {
        ...
        gui_create_dialog(GTK_MESSAGE_INFO, tmpbuf, GTK_WINDOW(appGUI->cal->ical_export_window));
    }
...
gchar tmpbuf[BUFFER_SIZE];
    ...
    gtk_box_pack_end (GTK_BOX (hbox4), remove_item_button, FALSE, FALSE, 0);
    ...
    gtk_box_pack_end (GTK_BOX (hbox4), clear_list_button, FALSE, FALSE, 4);
