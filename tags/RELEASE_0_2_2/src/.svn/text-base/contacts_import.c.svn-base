...
import_sel_cb (GtkWidget *widget, gpointer user_data) {
    ...
    if (config.import_type == IMPORT_TYPE_FILE) {
        ...
        if (!strlen(text)) {
            ...
            gui_create_dialog (GTK_MESSAGE_ERROR, tmpbuf, GTK_WINDOW(appGUI->cnt->import_sel_window));
            return;
        }
...
gchar tmpbuf[BUFFER_SIZE];
    ...
    if (config.default_stock_icons) {
        appGUI->cnt->contacts_browse_button = gui_stock_button (GTK_STOCK_DIRECTORY, FALSE);
    } else {
        appGUI->cnt->contacts_browse_button = gtk_button_new_from_stock (OSMO_STOCK_BUTTON_OPEN);
    }
    ...
    bluetooth_channel_spinbutton_adj = gtk_adjustment_new (1, 0, 100, 1, 10, 10);
    ...
    usb_interface_spinbutton_adj = gtk_adjustment_new (1, 0, 100, 1, 10, 10);
    ...
    appGUI->cnt->import_button = gtk_button_new_from_stock (OSMO_STOCK_BUTTON_CONTACTS_IMPORT);
...
gchar tmpbuf[BUFFER_SIZE];
    ...
    if (!k) {
        gui_create_dialog(GTK_MESSAGE_WARNING, _("Nothing to import."), GTK_WINDOW(appGUI->cnt->import_window));
        return;
    }
    ...
    gui_create_dialog (GTK_MESSAGE_INFO, tmpbuf, GTK_WINDOW (appGUI->cnt->import_window));
    ...
    if (g_access (filename, R_OK) != -1) {
        ...
        if (g_file_get_contents (filename, &appGUI->cnt->file_buffer, &appGUI->cnt->file_length, NULL) == TRUE) {
            ...
            if (n_records) {
                ...
                appGUI->cnt->current_record_spinbutton_adj = gtk_adjustment_new (1, 1, n_records, 1, 10, 10);
                ...
                gui_create_dialog(GTK_MESSAGE_ERROR, _("No records found in selected file."), GTK_WINDOW(appGUI->main_window));
                return FALSE;
            }

        } else {
            gui_create_dialog(GTK_MESSAGE_ERROR, _("Cannot read file."), GTK_WINDOW(appGUI->main_window));
            return FALSE;
        }
    } else {
        gui_create_dialog(GTK_MESSAGE_ERROR, _("Cannot open file."), GTK_WINDOW(appGUI->main_window));
        return FALSE;
    }
