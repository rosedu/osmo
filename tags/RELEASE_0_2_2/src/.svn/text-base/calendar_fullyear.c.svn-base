...
guint current_year;
	...
	current_year = get_gdate_current_year ();
	gtk_spin_button_set_value (GTK_SPIN_BUTTON (appGUI->cal->fy_spinbutton), current_year);
}
...
guint i, month, year;
	...
	if (year == get_gdate_current_year ()) {
...
void
browse_ical_events_cb (GtkWidget *widget, gpointer user_data)
{
	GUI *appGUI = (GUI *) user_data;
