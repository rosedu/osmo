 ...
 * organizer by pasp@users.sourceforge.net
 */
...
check_date(guint year, guint mm, guint dd) {
    ...
    if ((dd < 1) || (dd > month_length[g_date_is_leap_year(year)][mm])) return FALSE;
    return TRUE;
}
...
calc_days(guint year, guint mm, guint dd) {
    ...
    if ((dd < 1) || (dd > month_length[(lp = g_date_is_leap_year(year))][mm])) return(0L);
...
gui_calendar_class_init (GuiCalendarClass *class) {
    ...
    /**
     * GuiCalendar:week-start-monday:
     *
     * Determines whether week starts at monday.
     *
     */
...
gui_calendar_init (GuiCalendar *calendar) {
    ...
    for (i=0;i<31;i++) {
        ...
        calendar->tc_marked_date[i] = FALSE;
    }
    calendar->num_marked_dates = 0;
    calendar->tc_num_marked_dates = 0;
...
calendar_set_month_next (GuiCalendar *calendar) {
    ...
    month_len = month_length[g_date_is_leap_year (calendar->year)][calendar->month + 1];
...
calendar_compute_days (GuiCalendar *calendar) {
    ...
    ndays_in_month = month_length[g_date_is_leap_year (year)][month];
    ...
    if (month > 1) {
        ndays_in_prev_month = month_length[g_date_is_leap_year (year)][month-1];
    } else {
        ndays_in_prev_month = month_length[g_date_is_leap_year (year)][12];
    }
...
calendar_set_month_prev (GuiCalendar *calendar) {
    ...
    if (month_len < calendar->selected_day) {
        ...
        if (calendar->selected_day < 0)
            calendar->selected_day = calendar->selected_day + 1 + month_length[g_date_is_leap_year (calendar->year)][calendar->month + 1];
        gui_calendar_select_day (calendar, calendar->selected_day);
    }
...
calendar_paint_day (GuiCalendar *calendar,
    ...
    /*  layout = gtk_widget_create_pango_layout (widget, g_locale_to_utf8 (buffer, -1, NULL, NULL, NULL));*/
 ...
 *
 * Return value: a newly #GuiCalendar widget
 **/
...
/**
 * gui_calendar_get_display_options:
 * @calendar: a #GuiCalendar
 *
...
/**
 * gui_calendar_set_display_options:
 * @calendar: a #GuiCalendar
 ...
 **/
void
gui_calendar_clear_marks (GuiCalendar *calendar) {
    guint day;
...
void
gui_calendar_tc_clear_marks (GuiCalendar *calendar) {
    guint day;

    g_return_if_fail (GUI_IS_CALENDAR (calendar));

    for (day = 0; day < 31; day++) {
        calendar->tc_marked_date[day] = FALSE;
    }
 ...
 **/
gboolean
gui_calendar_mark_day (GuiCalendar *calendar,
                       guint        day) {
    g_return_val_if_fail (GUI_IS_CALENDAR (calendar), FALSE);
...
gboolean
gui_calendar_tc_mark_day (GuiCalendar *calendar,
                          guint        day) {
    g_return_val_if_fail (GUI_IS_CALENDAR (calendar), FALSE);

    if (day >= 1 && day <= 31 && calendar->tc_marked_date[day-1] == FALSE) {
        calendar->tc_marked_date[day - 1] = TRUE;
        calendar->tc_num_marked_dates++;
        calendar_invalidate_day_num (calendar, day);
    }
...
gboolean
gui_calendar_mark_day_color (GuiCalendar *calendar,
                             guint        day,
                             gchar       *color_str) {
    g_return_val_if_fail (GUI_IS_CALENDAR (calendar), FALSE);
 ...
 **/
gboolean
gui_calendar_unmark_day (GuiCalendar *calendar,
                         guint        day) {
    g_return_val_if_fail (GUI_IS_CALENDAR (calendar), FALSE);
...
gboolean
gui_calendar_tc_unmark_day (GuiCalendar *calendar,
                            guint        day) {
    g_return_val_if_fail (GUI_IS_CALENDAR (calendar), FALSE);

    if (day >= 1 && day <= 31 && calendar->tc_marked_date[day-1] == TRUE) {
        calendar->tc_marked_date[day - 1] = FALSE;
        calendar->tc_num_marked_dates--;
        calendar_invalidate_day_num (calendar, day);
    }
 ...
 *
 * Obtains the selected date from a #GuiCalendar.
 **/
void
...
void
gui_calendar_set_header_color (GuiCalendar *calendar, const gchar *color) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    gdk_color_parse(color, &calendar->header_color);
}

void
gui_calendar_set_weekend_color (GuiCalendar *calendar, const gchar *color) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    gdk_color_parse(color, &calendar->weekend_color);
}

void
gui_calendar_set_selector_color (GuiCalendar *calendar, const gchar *color) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    gdk_color_parse(color, &calendar->selector_color);
}

void       
gui_calendar_set_selector_alpha (GuiCalendar *calendar, gint alpha_value) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    calendar->selector_alpha = alpha_value;
}

void
gui_calendar_set_day_note_marker_type (GuiCalendar *calendar, const gchar symbol) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    calendar->mark_sign = symbol;
}
...
void
gui_calendar_set_event_marker_color (GuiCalendar *calendar, const gchar *color) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
...
void
gui_calendar_set_today_marker_color (GuiCalendar *calendar, const gchar *color) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    gdk_color_parse(color, &calendar->today_marker_color);
}

void       
gui_calendar_set_today_marker_type (GuiCalendar *calendar, gint marker_type) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    calendar->today_marker_type = marker_type;
}

void       
gui_calendar_set_today_marker_alpha (GuiCalendar *calendar, gint alpha_value) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
    calendar->today_marker_alpha = alpha_value;
}

void       
gui_calendar_set_event_marker_type (GuiCalendar *calendar, gint marker_type) {
    g_return_if_fail (GUI_IS_CALENDAR (calendar));
