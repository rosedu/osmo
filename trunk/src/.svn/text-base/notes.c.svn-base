...
GtkTreeViewColumn   *col;
	...
	if (gtk_tree_view_column_get_visible(col) == TRUE) {
		gtk_tree_view_column_set_fixed_width (col, config.notes_column_idx_4_width - 
											  2 * utl_gui_get_sw_vscrollbar_width (appGUI->nte->scrolled_win));
	}
}
...
guint n_t_entries_editor = G_N_ELEMENTS (t_entries_editor);
	...
	g_signal_connect (G_OBJECT (gtk_ui_manager_get_widget (appGUI->nte->notes_uim_selector_widget, "/toolbar/preferences")), "clicked",
	                  G_CALLBACK (show_preferences_window_cb), appGUI);

	g_signal_connect (G_OBJECT (gtk_ui_manager_get_widget (appGUI->nte->notes_uim_selector_widget, "/toolbar/about")), "clicked",
