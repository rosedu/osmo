...
static void export_clicked( GtkWidget *widget, exportData *data)
{
	...
	*/	
	...
	//void *appGUI = (void *) data->appGUI;
	result = pthread_create (&thread, NULL, tasks_export_gcal, data);
	if (result != 0) {
...
GtkTreeViewColumn   *col;
	...
	if (gtk_tree_view_column_get_visible(col) == TRUE) {
		gtk_tree_view_column_set_fixed_width (col, config.tasks_column_idx_5_width - 
											  2 * utl_gui_get_sw_vscrollbar_width (appGUI->tsk->scrolled_win));
	}
}
...
GtkActionEntry entries[] = {
  ...
  { "prev_date", OSMO_STOCK_TASKS_PREV_DATE, _("Change due date to previous date"), NULL, _("Change due date to previous date"), NULL },
  { "next_date", OSMO_STOCK_TASKS_NEXT_DATE, _("Change due date to next date"), NULL, _("Change due date to next date"), NULL },
#if 0
...
#endif /* PRINTING_SUPPORT */
	{ "preferences", OSMO_STOCK_PREFERENCES, _("Preferences"), NULL, _("Preferences"), NULL},
	{ "about", OSMO_STOCK_ABOUT, _("About"), NULL, _("About"), NULL},
};
...
guint n_entries = G_N_ELEMENTS (entries);
    ...
    g_signal_connect (G_OBJECT(gtk_ui_manager_get_widget (appGUI->tsk->tasks_uim_widget, "/toolbar/prev_date")), "clicked", 
                      G_CALLBACK(tasks_change_due_date_to_prev_date_cb), appGUI);
    g_signal_connect (G_OBJECT(gtk_ui_manager_get_widget (appGUI->tsk->tasks_uim_widget, "/toolbar/next_date")), "clicked", 
                      G_CALLBACK(tasks_change_due_date_to_next_date_cb), appGUI);
#ifdef HAVE_LIBICAL
	...
	g_signal_connect (G_OBJECT (gtk_ui_manager_get_widget (appGUI->tsk->tasks_uim_widget, "/toolbar/preferences")), "clicked",
	                  G_CALLBACK (show_preferences_window_cb), appGUI);

	g_signal_connect (G_OBJECT (gtk_ui_manager_get_widget (appGUI->tsk->tasks_uim_widget, "/toolbar/about")), "clicked",
